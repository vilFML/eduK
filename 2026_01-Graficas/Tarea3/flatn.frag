...
void main()
{
    ...
    finalColor += computeDirectionalLight(normal, viewDir, u_dirLight);

    if (u_numPointLights > 0 && u_numPointLights <= MAX_POINT_LIGHTS) {
        for (int i = 0; i < u_numPointLights; i++)
            finalColor += computePointLight(normal, viewDir, u_pointLights[i]);
    }

    // Agregar nieve en la cima
    float snowHeight = 1.0; // Altura a partir de la cual empieza la nieve
    float blendZone = 0.2;   // Zona de transición para mezcla suave

    float snowFactor = clamp((fragPos.y - snowHeight) / blendZone, 0.0, 1.0);

    // Mezclamos el color con blanco dependiendo del factor
